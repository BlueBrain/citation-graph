...
def load_europmc_article_author_mapping(
    europmc_article_xmls_path: str, article_author_mapping: dict
):
    """
    ...
    for filename in os.listdir(europmc_article_xmls_path):
        if filename.endswith(".xml"):
            ...
            if file_key in article_author_mapping:
                article_author_mapping[file_key] = (
...
def main(
    ...
    article_author_xml_paths = glob(
        os.path.join(article_author_xml_path_doi, "*.xml")
    )
    article_author_xml_paths.extend(
        [
            path
            for path in glob(
                os.path.join(article_author_xml_path_author_name, "*.xml")
            )
            if path not in article_author_xml_paths
        ]
    )
    article_author_mapping = {}
    for article_xml_path in tqdm(
        article_author_xml_paths, desc="Mapping articles to author orcidids"
    ):
        ...
        orcidids = [
            orcid.text
            for orcid in element.findall(
                ".//common:path", namespaces=NAMESPACES
            )
        ]
        article_author_mapping[article_uid] = orcidids

    load_europmc_article_author_mapping(
        articles_europmc_xml_path, article_author_mapping
    )

    author_xml_paths = glob(os.path.join(authors_xml_path, "*.xml"))
    author_record_map = {}
    for xml_path in tqdm(
        author_xml_paths, desc="Collecting author orcid records"
    ):
    ...
    authors = []
    for author_id in tqdm(
        list(author_record_map.keys()), desc="Building authors.csv"
    ):
        ...
        if bool(re.match(r"\b\d{4}-\d{4}-\d{4}-\d{3}[0-9X]\b", author_id)):
            ...
            author = Author(
                uid=author_id, google_scholar_id=author_id, name=name
            )
    ...
    for article_uid in tqdm(
        article_author_mapping, desc="Building author_wrote_article.csv"
    ):
        for orcidid in article_author_mapping[article_uid]:
            author_wrote_article.append(
                AuthorWroteArticle(author_uid=orcidid, article_uid=article_uid)
            )
    author_wrote_article = [
    ...
    author_wrote_article.drop_duplicates(
        subset=["author_uid", "article_uid"], inplace=True
    )
    author_wrote_article.to_csv(
        os.path.join(output_dir, "author_wrote_article.csv"), index=False
    )
    ...
    for orcidid in tqdm(author_record_map, desc="Building affiliations"):
        ...
        author_institutions, affiliations = get_author_affiliations(
            orcidid, record
        )
    ...
    author_aff_institution = [
        affiliation.model_dump() for affiliation in author_aff_institution
    ]
    author_aff_institution = pd.DataFrame(author_aff_institution)
    author_aff_institution.sort_values(
        by=["author_uid", "institution_uid"], inplace=True
    )
    author_aff_institution.drop_duplicates(
        subset=["author_uid", "institution_uid"], inplace=True
    )
    author_aff_institution.to_csv(
        os.path.join(
            args.output_dir, "author_affiliated_with_institution.csv"
        ),
        index=False,
    )
    ...
    institutions.to_csv(
        os.path.join(args.output_dir, "institutions.csv"), index=False
    )
