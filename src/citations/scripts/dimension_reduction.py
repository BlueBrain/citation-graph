...
def fetch_embeddings(uri, user, password):
    ...
    with driver.session() as session:
        ...
        embeddings = [
            (record["uid"], record["embedding"]) for record in result
        ]
    driver.close()
    return embeddings
...
def main(args):
    ...
    uid_to_umap = generate_umap_coordinates(
        embeddings, args.n_components, args.random_state
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--user", type=str, default="neo4j", required=True, help="Neo4j user"
    )
    parser.add_argument(
        "--password",
        type=str,
